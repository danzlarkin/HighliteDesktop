...
export class ContextMenuOptions extends Plugin {
    ...
    start(): void {
        this.log("Started");
        this.enablePrioritizeAttackChanged(this.settings.prioritizeAttack!.value as boolean);
        this.enablePrioritizePickpocketChanged(this.settings.prioritizePickpocket!.value as boolean);
    }
    ...
    enablePrioritizePickpocketChanged() {
      if (this.settings.prioritizePickpocket?.value && this.settings.enable?.value) {
        this.contextMenuManager.SetGameWorldActionMenuPosition("Pickpocket", -1);
      } else {
        this.contextMenuManager.RemoveGameWorldActionMenuPosition("Pickpocket");
      }
    }

    enablePrioritizeAttackChanged() {
      if (this.settings.prioritizeAttack?.value && this.settings.enable?.value) {
        this.contextMenuManager.SetGameWorldActionMenuPosition("Attack", -1);
      } else {
