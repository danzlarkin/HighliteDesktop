...
export class EnhancedHPBars extends Plugin {
    ...
    constructor() {
        ...
        this.settings.centerWindow = {
            ...
            callback: () => {
                this.updatePosition(
                    this.settings.centerWindow!.value as boolean
                );
            },
        };
    }
    ...
    updatePosition(enabled: boolean) {
        ...
        if (enabled) {
            ...
            let combatHeader = document.getElementById(
                'highlite-combat-header'
            );
            if (!combatHeader) {
            ...
            let sideBySideContainer = document.getElementById(
                'highlite-side-by-side-container'
            );
            ...
            const combatHeader = document.getElementById(
                'highlite-combat-header'
            );
            const sideBySideContainer = document.getElementById(
                'highlite-side-by-side-container'
            );
    ...
    createTargetContainer() {
        ...
        document
            ...
            ?.appendChild(this.targetContainer);
        ...
        this.playerHealthBarBack.style.backgroundColor =
            'rgba(242, 67, 67, 0.5)';
        ...
        this.playerHealthBarFront.style.backgroundColor =
            'rgba(88, 162, 23, 1)';
    ...
    start(): void {
        ...
        if (
            ...
            document.getElementById('hs-screen-mask') !== null
        ) {
    ...
    GameLoop_draw() {
        ...
        if (
            ...
            !this.healthBarFront ||
            !this.playerNameDiv ||
            !this.playerHealthBarBack ||
            !this.playerHealthText ||
            !this.playerHealthBarFront
        ) {
            return;
        }
        ...
        const target =
            this.gameHooks.EntityManager.Instance.MainPlayer.CurrentTarget;
        if (target == this.playerTarget && this.playerAction == 'attack') {
            ...
            for (const [_id, npc] of npcs) {
                if (
                    npc.CurrentTarget &&
                    npc.CurrentTarget.EntityID === playerID
                ) {
                    this.targetContainer.style.visibility = 'visible';
