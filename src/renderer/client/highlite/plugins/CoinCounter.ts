...
export class CoinCounter extends Plugin {
    ...
    start(): void {
        this.log("Started");
        if (this.settings.enable!.value && this.isLoggedIn && !this.coinCounterUI) {
    ...
    createCoinCounterUI() : void {
        ...
        this.coinCounterUI = this.uiManager.createElement(UIManagerScope.ClientInternal);
    ...
    GameLoop_update(): void {
        ...
        const coins = bankItems.find((item: any) => item != null && item._id === 6);
        if (coins) {
        ...
        const inventoryCoins = inventoryItems.find((item: any) => item != null && item._id === 6);
        if (inventoryCoins) {
