  ...
  setItem(keyName, value) {
    ...
    return new Promise((resolve, reject) => {
      setRequest.onsuccess = (event) => {
        console.debug(`[Highlite Loader] setItem set Key: ${keyName} to Value: ${value}`);
        resolve(true);
      };
      setRequest.onerror = (event) => {
        console.warn(`[Highlite Loader] setItem could not set Key: ${keyName} to Value: ${value}`);
        resolve(false);
      };
    });
  }
  getItem(keyName) {
    ...
    return new Promise((resolve, reject) => {
      getRequest.onsuccess = (event) => {
        console.debug(`[Highlite Loader] getItem retrieved Key: ${keyName} with Value: ${getRequest.result}`);
